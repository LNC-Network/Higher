    ...
    <div className='group relative rounded-xl border border-gray-800 bg-[#0c111c] p-5 hover:border-blue-500 transition-all duration-300 mt-5 shadow-md'>
      <div className='flex flex-col md:flex-row justify-between gap-4'>
        ...
        <div className='flex-1'>
          {/* Title */}
          <h3 className='text-2xl font-bold text-white mb-2 group-hover:text-blue-400 transition-colors text-left'>
            {article.title}
          </h3>
          ...
          <div className='flex flex-wrap gap-2'>
            {article.tags.map((tag, index) => (
              <span
                key={index}
                className='bg-gray-800 text-blue-300 text-xs px-2.5 py-1 rounded-full border border-gray-700 hover:border-blue-500 transition'>
                #{tag}
              </span>
            ))}
          </div>
        ...
        {article.coverImage && (
          <div className='shrink-0 self-start'>
            <div className='relative rounded-lg overflow-hidden border border-gray-700 shadow-lg hover:scale-105 transition-transform duration-300'>
              {/* <Image src={article.coverImage} alt={article.title} fill className="object-cover rounded-lg w-28 h-14" /> */}
            </div>
          </div>
        )}
      </div>
