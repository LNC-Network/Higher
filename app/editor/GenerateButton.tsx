...
const GenerateButton = () => {
	...
	const handleGenerate = async (close: () => void) => {
		...
		try {
			...
			if (response) {
				const { content, tags } = response;
				const generated = content || 'No output generated.';
					...
					<div className='space-y-4'>
						...
						<div className='relative'>
							<input
								...
								className='w-full p-3 pr-10 rounded-lg  backdrop-blur border  placeholder:text-gray-300 text-white focus:ring-1 outline-none'
							/>
							{prompt && (
								<button
									className='absolute top-1/2 right-3 -translate-y-1/2 text-gray-300 hover:text-red-400'
									onClick={() => setPrompt('')}>
						...
						<div className='flex justify-evenly'>
							...
							<Button
								...
								className='w-1/3 bg-gradient-to-br from-purple-600 to-indigo-600 hover:brightness-110 text-white font-semibold rounded-xl shadow-lg'>
