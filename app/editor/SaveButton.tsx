...
export default function SaveButton() {
	...
	const fileSize = useMemo(() => {
		if (isEmpty) return 0;
		const content =
			fileType === 'json' ? JSON.stringify({ content: message }, null, 2) : message;
				...
				<div className='mt-4 space-y-4'>
					<div>
						...
						<Input
							...
							className='bg-slate-700 text-white border border-white/20'
						/>
					</div>
					...
					{!isEmpty && (
						<p className='text-sm text-slate-400'>
							Estimated file size: {(fileSize / 1024).toFixed(2)} KB
						</p>
					)}

					{isEmpty && (
						<p className='text-sm text-red-400'>
							Editor is empty. Please add content to save.
						</p>
					)}
				</div>
