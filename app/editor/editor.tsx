	...
	const renderMarkdown = (content: string) => {
		const components: Components = {
			h1: ({ children }: React.PropsWithChildren<{}>) => (
				<h1 className='text-2xl font-bold my-4 dark:text-gray-100'>{children}</h1>
			),
			h2: ({ children }: React.PropsWithChildren<{}>) => (
				<h2 className='text-xl font-bold my-3 dark:text-gray-100'>{children}</h2>
			),
			h3: ({ children }: React.PropsWithChildren<{}>) => (
				<h3 className='text-lg font-bold my-2 dark:text-gray-100'>{children}</h3>
			),
			p: ({ children }: React.PropsWithChildren<{}>) => (
				<p className='my-2 dark:text-gray-200'>{children}</p>
			),
			ul: ({ children }: React.PropsWithChildren<{}>) => (
				<ul className='list-disc pl-6 my-2 dark:text-gray-200'>{children}</ul>
			),
			ol: ({ children }: React.PropsWithChildren<{}>) => (
				<ol className='list-decimal pl-6 my-2 dark:text-gray-200'>{children}</ol>
			),
			blockquote: ({ children }: React.PropsWithChildren<{}>) => (
				<blockquote className='border-l-4 border-gray-300 pl-4 italic my-2 dark:border-gray-600 dark:text-gray-300'>
					{children}
				</blockquote>
			),
			code: ({
				...
				inline ? (
					<code className='bg-gray-100 dark:bg-gray-800 px-1 py-0.5 rounded text-sm dark:text-cyan-300'>
						{children}
					</code>
				) : (
			...
			tr: ({ children }: React.PropsWithChildren<{}>) => (
				<tr className='dark:border-gray-700'>{children}</tr>
			),
			th: ({ children }: React.PropsWithChildren<{}>) => (
				<th className='px-4 py-2 text-left border border-gray-300 dark:border-gray-700 dark:text-gray-200'>
					{children}
				</th>
			),
			td: ({ children }: React.PropsWithChildren<{}>) => (
				<td className='px-4 py-2 border border-gray-300 dark:border-gray-700 dark:text-gray-300'>
					{children}
				</td>
			),
			...
			<div className='border rounded-lg shadow-sm dark:border-gray-700 flex flex-col flex-1 dark:bg-gray-900'>
				...
				<div className='border-b p-2 bg-muted/30 flex items-center gap-1 overflow-x-auto dark:bg-gray-800 dark:border-gray-700'>
					{[
						...
						return (
							<Button
								...
								variant='ghost'
								size='icon'
								onClick={() => insertMarkdown(tag as string)}
								title={tag as string}
					...
					<TabsList className='h-8 bg-transparent dark:bg-gray-800'>
						<TabsTrigger
							value='edit'
							className='flex items-center gap-1 px-2 h-8 data-[state=active]:bg-background dark:text-gray-300 dark:data-[state=active]:bg-gray-900 dark:data-[state=active]:text-white'>
				...
				<div className='flex-1 overflow-hidden'>
					<TabsContent value='edit' className='h-full'>
						<textarea
							...
							className='w-full h-full p-4 font-mono text-sm resize-none focus:outline-none dark:bg-gray-900 dark:text-gray-100 dark:caret-cyan-400'
					...
					<TabsContent value='split' className='h-full'>
						<div className='grid grid-cols-2 h-full'>
							<textarea
								...
								placeholder='Type your markdown here...'
							/>
							<div className='max-w-none p-4 overflow-auto dark:bg-gray-900'>
				...
				<div className='flex items-center justify-between p-2 border-t dark:border-gray-700'>
					<div className='flex flex-wrap items-center gap-2'>
						<span className='flex items-center gap-2 text-lg text-muted-foreground dark:text-white rounded-full border border-gray-500 bg-linear-to-br from-blue-600/50 via-violet-500/20 to-fuchsia-600/50 px-4 py-1 font-mono'>
							<TagsIcon />
							AI Generated Tags :
						</span>
